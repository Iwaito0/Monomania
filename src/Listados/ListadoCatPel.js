...
export default function ListadoCatPel() {
    ...
    async function listadoPel() {
        ...
        let response = await fetch(`php/proyectoFinal/Pelicula/getDatosListadoPeliculas.php`, {
            body: datos,
            method: 'POST'
        });
    ...
    return (
        <>
            ...
            {visibilidadListado && (
                <>
                    <TableContainer component={Paper} className="mt-3 mx-0">
                        <Table sx={{ minWidth: 700 }} aria-label="customized table">
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell align="center" className="tablaList" sx={{ color: grey[50] }}>Fecha de estreno</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {datosBDListado.map((row) => (
                                    <StyledTableRow key={row.idPelicula}>
                                        <StyledTableCell align="center"> {row.titulo}</StyledTableCell>
                                        <StyledTableCell align="center">{row.tituloIngles}</StyledTableCell>
                                        <StyledTableCell align="center">{row.fechaEstreno}</StyledTableCell>
                                    </StyledTableRow>
                                ))}
                            </TableBody>
                        </Table>
                ...
                <div className='col-12  d-flex justify-content-center formularioIniReg'>
                    <div className="card" style={{ width: '25rem' }}>
                        <div className="card-body">
                            <h2 className="card-title text-center mt-3">Listar peliculas</h2>
                            ...
                            <button onClick={listadoPel} className="btn btn-dark mt-4 col-12" disabled={btnDispo}>Aceptar</button>
                        </div>
                    </div>
                </div>
